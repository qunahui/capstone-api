  ...
  await Promise.all(
    linkedIds.map(async (linkedId) => {
      if (linkedId.platform === 'sendo') {
        ...
        if (data) {
          sendoProduct = await SendoProduct.findOne({ _id: data.productId })
            .populate('variants')
            .lean();

          sendoProduct.variants = sendoProduct.variants.map(
            (matchedVariant) => {
        ...
        const lazadaProduct = await LazadaProduct.findOne({
          _id: lazadaVariant.productId,
        })
          .populate('variants')
          .lean();
        const storage = await Storage.findOne(
        ...
        lazadaProduct.variants = lazadaProduct.variants.map(
          (matchedVariant) => {
            if (matchedVariant._id.toString() === linkedId.id) {
              return (matchedVariant = {
                ...matchedVariant,
